...
class CommandR(Command):
	...
	@staticmethod
	def display():
		foreground = QtGui.QBrush(QtGui.QColor(139, 71, 137))
		background = QtGui.QBrush(QtGui.QColor(255, 187, 193))	
	...
	def execute(self, runtime):
		...
		str_command = str("\"" + Options.R_Location + "\" --file=" + filename)
		p = subprocess.Popen(str_command, shell=True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
		...
		if pf == 'Windows':						
			...
			f.write(self.code)
			f.close()
			shellscript = "cd " + str(Options.R_Location) + "\n R --file=\"Rscript.R\""			
			f = open( "script.bat", 'w' )
			f.write(shellscript)
			f.close()			
		elif pf == 'Darwin':
			...
			f.close()			
			subprocess.call(['chmod', '0777', filename])		
		p = subprocess.Popen("script.bat", shell=True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)					
		(stdout, stderr) = p.communicate()				
