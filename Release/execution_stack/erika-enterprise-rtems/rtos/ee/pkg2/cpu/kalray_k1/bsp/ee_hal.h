...
EE_INLINE__ EE_UREG EE_k1_get_cpu_id ( void ) {
  return __k1_get_cpu_id();
}

EE_INLINE__ EE_CORE_ID EE_get_curr_core_id ( void ) {
  return  EE_k1_get_cpu_id ();
}

/* Disable/Enable Interrupts */
EE_INLINE__ void EE_k1_disableIRQ ( void )
{
  __k1_interrupt_disable();
}

EE_INLINE__ void EE_k1_enableIRQ ( void )
{
  __k1_interrupt_enable();
}

/* Suspend/Resume Interrupts */
EE_INLINE__ EE_FREG EE_k1_suspendIRQ ( void )
{
  EE_FREG flags = __builtin_k1_get(_K1_SFR_PS);
  EE_k1_disableIRQ();
  return flags;
}

EE_INLINE__ void EE_k1_resumeIRQ ( EE_FREG flags )
{
  __builtin_k1_set(_K1_SFR_PS, flags);
}

EE_INLINE__ EE_FREG EE_k1_isIRQEnabled ( void )
{
  return (__k1_interrupt_save() != 0U);
}

/* New functions needed for Resource sharing with ISR  */
EE_INLINE__ EE_isr2_prio EE_k1_get_int_prio ( void )
{
  return (EE_isr2_prio)__k1_interrupt_level_get();
}

EE_INLINE__ void EE_k1_set_int_prio ( EE_isr2_prio prio )
{
  __k1_interrupt_level_set(prio);
}
