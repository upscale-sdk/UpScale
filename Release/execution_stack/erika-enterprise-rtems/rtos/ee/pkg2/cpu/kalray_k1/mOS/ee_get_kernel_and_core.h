...
EE_INLINE__ EE_KDB EE_CONST * EE_lock_and_get_kernel ( void ) {
  EE_lock_kernel ();
  return EE_get_kernel ();
}
...
EE_INLINE__ void  EE_lock_core_id ( EE_CORE_ID core_id ) {
  EE_k1_spin_lock(&KCB_WJ.core_ctrls[core_id].lock);
}

EE_INLINE__ void  EE_lock_core ( EE_CDB * const p_CDB ) {
  /* C single inheritance */
  EE_k1_spin_lock(&((EE_CCB_WL *)p_CDB->p_CCB)->lock);
}
...
EE_INLINE__ void EE_unlock_core_id ( EE_CORE_ID core_id ) {
  EE_CCB_WL * const p_CCB_wl = &KCB_WJ.core_ctrls[core_id];
  EE_k1_spin_unlock(&p_CCB_wl->lock);
}

EE_INLINE__ void  EE_unlock_core ( EE_CDB * const p_CDB ) {
  /* C single inheritance */
  EE_k1_spin_unlock(&((EE_CCB_WL *)p_CDB->p_CCB)->lock);
}

EE_INLINE__ void EE_unlock_curr_core ( void ) {
  EE_unlock_core_id ( EE_get_curr_core_id() );
}
