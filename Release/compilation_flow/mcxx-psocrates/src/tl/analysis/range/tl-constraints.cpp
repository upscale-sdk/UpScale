...
namespace {
    class AccessesVisitor : public Nodecl::ExhaustiveVisitor<void>
    {
    private:
        bool _contains_accesses;

    public:
        AccessesVisitor()
            : _contains_accesses(false)
        {}

        bool contains_accesses()
        {
            return _contains_accesses;
        }

        void visit(const Nodecl::Dereference& n)
        {
            _contains_accesses = true;
        }
    ...
    bool contains_accesses(const NBase& n)
    {
        AccessesVisitor av;
        av.walk(n);
        return av.contains_accesses();
    }
}
    ...
    void ConstraintBuilder::visit_assignment(const NBase& lhs, const NBase& rhs)
    {
        ...
        else if (contains_accesses(rhs))
        {   // We cannot know the value, so we assign a range of [-inf, +inf]
            val = Nodecl::Range::make(minus_inf.shallow_copy(),
                                      plus_inf.shallow_copy(),
                                      const_value_to_nodecl(zero), t);
        }
