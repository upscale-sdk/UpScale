...
typedef struct compilation_process_tag
{
    ...
    // Used for sublinking
    const char *sublinked_output_filename;
...
typedef enum multibinary_method_tag
{
    MULTIBINARY_NONE = 0,
    MULTIBINARY_MPPA
} multibinary_method_t;

typedef struct multibinary_tag
{
    multibinary_method_t method;
} multibinary_t;
