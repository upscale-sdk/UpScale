...
"  --sublink-output=<filename>\n" \
"                           When doing sublinking, forces the sublink\n" \
"                           target to be <filename>. Otherwise a name\n" \
"                           is derived from the main output\n" \
"\n" \
...
struct command_line_long_options command_line_long_options[] =
{
    ...
    {"sublink-output", CLP_REQUIRED_ARGUMENT, OPTION_SUBLINK_OUTPUT },
    // sentinel
    {NULL, 0, 0}
};
...
static void multibinary_creation(void);
...
int main(int argc, char* argv[])
{
    ...
    // Multibinary processing
    multibinary_creation();
                ...
                argc, argv))
    {
        ...
        else 
        {
            ...
            switch (parameter_info.value)
            {
                ...
                case OPTION_SUBLINK_OUTPUT:
                    {
                        if (compilation_process.sublinked_output_filename != NULL)
                        {
                            fprintf(stderr, "%s: sublink output file specified twice\n",
                                    compilation_process.exec_basename);
                            return 1;
                        }
                        compilation_process.sublinked_output_filename = uniquestr(parameter_info.argument);
                        break;
                    }
                default:
                    {
        ...
        compilation_configuration_t* target_configuration)
{
    ...
    struct tags_and_outputs
    {
        int tag;
        const char* output_name;
    }  *tags_and_outputs = NULL;
    int num_tags_and_output = 0;
    ...
    for (i = 0; i < num_multifile_profiles; i++)
    {
        ...
        else
        {
            ...
            const char* current_sublinked_output = compilation_process.sublinked_output_filename;
            int tag = multifile_profiles[i].tag;
            if (current_sublinked_output == NULL)
            {
            ...
            // Warn if we are going to overwrite an existing tag
            int j;
            for (j = 0; j < num_tags_and_output; j++)
            {
                if (strcmp(tags_and_outputs[j].output_name, current_sublinked_output) == 0)
                {
                    fprintf(stderr,
                            "warning: sublinking of files with tag %d will overwrite the file '%s' "
                            "created during sublinking of files with tag %d\n",
                            tag,
                            current_sublinked_output,
                            tags_and_outputs[j].tag);
                }
            }
            P_LIST_ADD(tags_and_outputs, num_tags_and_output,
                    ((struct tags_and_outputs){tag, current_sublinked_output}));
...
static void multibinary_creation(void)
{
    ...
    if (CURRENT_CONFIGURATION->multibinary.method == MULTIBINARY_NONE)
        return;

    switch (CURRENT_CONFIGURATION->multibinary.method)
    {
        case MULTIBINARY_MPPA:
            {
                int i;
                for (i = 0; i < compilation_process.num_subgoals; i++)
                {
                    if (strcmp(compilation_process.subgoals[i].configuration->configuration_name,
                                "psocratescc-cluster") != 0)
                    {
                        fprintf(stderr, "MPPA unexpected profile name '%s' for subgoal, "
                                "it should be 'psocratescc-cluster'\n",
                                compilation_process.subgoals[i].configuration->configuration_name);
                        return;
                    }
                }

                int num_args = 2 * compilation_process.num_subgoals /* --cluster X1 */
                    + 1 /* --boot=X2 */
                    + 2 /* -T X3 */
                    + 1 /* NULL */;
                ...
                int j = 0;
                for (i = 0; i < compilation_process.num_subgoals; i++, j++)
                {
                    args[j] = UNIQUESTR_LITERAL("--cluster");
                    j++;
                    args[j] = compilation_process.subgoals[i].linked_subgoal_filename;
                }

                uniquestr_sprintf(&args[j], "--boot=%s",
                        compilation_process.linked_output_filename);
                j++;

                args[j] = UNIQUESTR_LITERAL("-T");
                j++;
                const char* multibinary_filename = NULL;
                uniquestr_sprintf(&multibinary_filename, "%s.mpk",
                        compilation_process.linked_output_filename);
                args[j] = multibinary_filename;

                // k1-create-multibinary complains if the file already exists,
                // remove it now
                remove(multibinary_filename);

                if (execute_program("k1-create-multibinary", args) != 0)
                {
                    fatal_error("Error when invoking 'k1-create-multibinary'");
                }

                fprintf(stderr, "MPPA multibinary created in '%s'\n", multibinary_filename);
                break;
            }
        default:
            {
                internal_error("Unknown multibinary method %d\n", CURRENT_CONFIGURATION->multibinary.method);
            }
    }
}
