...
int config_set_multibinary(compilation_configuration_t* config, const char* index UNUSED_PARAMETER, const char* value)
{
    if (0);
    else if (strcmp(value, "mppa") == 0)
    {
        config->multibinary.method = MULTIBINARY_MPPA;
    }
    else
    {
        fprintf(stderr, "Ignoring unknown value '%s' for multibinary option\n", value);
        return 1;
    }
    return 0;
}
