...
namespace TL { namespace OpenMP {
    ...
    void Base::omp_target_handler_post(TL::PragmaCustomStatement ctr)
    {
        ...
        // Obtaining the priority-id if present
        {
            TL::PragmaCustomClause priority_id = pragma_line.get_clause("priority_id");
            if (priority_id.is_defined())
            {
                TL::ObjectList<Nodecl::NodeclBase> expr_list = priority_id.get_arguments_as_expressions();
                if (expr_list.size() == 1)
                {
                    device_data_environment.append(
                            Nodecl::OpenMP::PriorityId::make(expr_list[0])
                            );
                }
                else
                {
                    error_printf_at(pragma_line.get_locus(), "invalid number of arguments in 'priority_id' clause\n");
                }
            }
        }
