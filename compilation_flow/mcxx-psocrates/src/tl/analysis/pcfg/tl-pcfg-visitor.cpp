    ...
    ObjectList<Node*> PCFGVisitor::visit(const Nodecl::ForStatement& n)
    {
        ...
        if(next != NULL)
        {
            ...
            {   // When there is no condition,
                //    - connect with the 'next' inner statements, if there are
                //    - connect with the 'entry' node, otherwise
                const ObjectList<Node*>& entry_children = entry_node->get_children();
                if (entry_children.size() > 0)
                    _pcfg->connect_nodes(last_next, entry_children[0]);
                else
                    _pcfg->connect_nodes(last_next, entry_node);
            }
        }
        else
        {
            ...
            {   // When there is no condition and no next,
                //     - connect with the loop last inner node, if there are
                //     - connect with the entry node, otherwise
                const ObjectList<Node*>& entry_children = entry_node->get_children();
                if (entry_children.size() > 0)
                    _pcfg->connect_nodes(_utils->_last_nodes, entry_children[0]);
                else
                    _pcfg->connect_nodes(_utils->_last_nodes, entry_node);
            }
        }
...
//         walk(n.get_specs());      // Specs are not used for any analysis
    ...
    ObjectList<Node*> PCFGVisitor::visit(const Nodecl::OpenMP::TaskId& n)
    {
        _utils->_pragma_nodes.top()._clauses.append(n);
        return ObjectList<Node*>();
    }
