...
//                 );
            ...
            {   // n=[lb1, ub1], m=[v]
                cond_part = Nodecl::LogicalAnd::make(
                    ...
                    Nodecl::LowerOrEqualThan::make(m.shallow_copy(), n.as<Nodecl::Range>().get_upper().shallow_copy(), t),
                    t
               );
            }
        }
        else
        {
            ...
            {   // n=[v], m=[lb1, ub1]
                cond_part = Nodecl::LogicalAnd::make(
                    ...
                    Nodecl::LowerOrEqualThan::make(n.shallow_copy(), m.as<Nodecl::Range>().get_upper().shallow_copy(), t),
                    t
               );
            }
            else
...
//                     else
            ...
            // If only one of the subscripts is different, then the dependency does not exist
...
// match_array_subscripts_end:
    ...
    NBase TaskSyncTunning::match_dependencies(Node* source, Node* target)
    {
        ...
        // 2.1.- (WAR) Match source(in) with target(out, inout)
