...
namespace {
    ...
    // @param lhs true simplifies lhs; false simplifies rhs
    void simplify_boundary(
            NBase n,
            const std::set<CGNode*>& future_grandparents,
            bool lhs)
    {
        ObjectList<NBase> syms = Nodecl::Utils::get_all_memory_accesses(n);
        for (ObjectList<NBase>::iterator it = syms.begin(); it != syms.end(); ++it)
        {
                    ...
                    itf != future_grandparents.end(); ++itf)
            {
                ...
                {   // Replace the symbol by its constant value, if it has
                    ...
                    if (itf_val.is_constant())
                    {
                        ...
                        Nodecl::Utils::nodecl_replace_nodecl_by_structure(n, *it, itf_val_const);
                    }
                    else if (itf_val.is<Nodecl::Range>())
                    {
                        if (lhs)
                        {
                            NBase lb = itf_val.as<Nodecl::Range>().get_lower();
                            ...
                            {
                                NBase lb_const = const_value_to_nodecl(lb.get_constant());
                                Nodecl::Utils::nodecl_replace_nodecl_by_structure(n, *it, lb_const);
                            }
                        }
                        else
                        {
                            NBase ub = itf_val.as<Nodecl::Range>().get_upper();
                            if (ub.is<Nodecl::Analysis::PlusInfinity>())
                            {
                                n.replace(ub);
                                // It cannot go any lower
                                return;
                            ...
                            {
                                NBase ub_const = const_value_to_nodecl(ub.get_constant());
                                Nodecl::Utils::nodecl_replace_nodecl_by_structure(n, *it, ub_const);
                            }
                        }
                    }
                }
        ...
        n = inf_calc.compute(n);
    }

    void simplify_range_intersection_operator(
            NBase& n,
            const std::set<CGNode*>& future_grandparents)
    {
        if (n.is<Nodecl::Range>())
        {
            Nodecl::Range tmp = n.as<Nodecl::Range>();
            NBase lb = tmp.get_lower();
            simplify_boundary(lb, future_grandparents, /*lb*/true);
            NBase ub = tmp.get_upper();
            simplify_boundary(ub, future_grandparents, /*lb*/false);
            Nodecl::Range new_range = Nodecl::Range::make(lb, ub, const_value_to_nodecl(zero), n.get_type());
            n.replace(new_range);
        }
        else if (n.is<Nodecl::Analysis::RangeUnion>())
        {
            Nodecl::Analysis::RangeUnion tmp = n.as<Nodecl::Analysis::RangeUnion>();
            NBase lhs = tmp.get_lhs();
            NBase rhs = tmp.get_rhs();
            simplify_range_intersection_operator(lhs, future_grandparents);
            simplify_range_intersection_operator(rhs, future_grandparents);
            n.replace(Utils::range_union(lhs, rhs));
        }
        else if (n.is<Nodecl::Analysis::RangeIntersection>())
        {
            Nodecl::Analysis::RangeIntersection tmp = n.as<Nodecl::Analysis::RangeIntersection>();
            NBase lhs = tmp.get_lhs();
            NBase rhs = tmp.get_rhs();
            simplify_range_intersection_operator(lhs, future_grandparents);
            simplify_range_intersection_operator(rhs, future_grandparents);
            n.replace(Utils::range_intersection(lhs, rhs));
        }
        else
        {
            internal_error("Unexpected node type '%s' while simplifying range intersection", ast_print_node_type(n.get_kind()));
        }
    }
}
                ...
                switch (parent_type)
                {
                    ...
                    case __Intersection:
                    {
                        ...
                        {   // Simplify the intersection range as possible
                            // First replace all symbol by its constant values
                            simplify_range_intersection_operator(op2, future_grandparents);
        ...
        while (!worklist.empty())
        {
            ...
            if (n->is_graph_node())
            {
                ...
                if (exit_parents.size()==0
                    || (exit_parents.size() == 1 && exit_parents[0]->is_return_node()))
                {
                    treated.insert(n);
                    continue;
                }
