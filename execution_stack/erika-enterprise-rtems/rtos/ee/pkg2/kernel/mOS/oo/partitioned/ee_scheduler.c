  ...
  EE_CDB EE_CONST * p_CDB )
{
  ...
  EE_SN  * const           p_rq_first = p_CCB->p_rq_first;
  ...
  if ( p_rq_first != NULL ) {
    ...
    p_CCB->p_rq_first             = p_rq_first->p_next;
    ...
    EE_BOOL preemption_point )
{
  ...
  if ( p_task_activated_tcb->residual_activation > 0U ) {
    ...
    rq_head_changed = EE_schedule_data_priority_insert (&p_CCB->p_rq_first,
      EE_scheduler_sn_alloc(p_CCB), p_task_activated);
    ...
    if ( rq_head_changed ) {
        ...
        p_task_activated_tcb->current_prio) )
      {
        ...
        if ( p_task_running != p_CDB->p_idle_task ) {
          (void)EE_schedule_data_priority_insert ( &p_CCB->p_rq_first,
            p_task_running_sn, p_task_running );
        }

        EE_unlock_core(p_CDB);
  ...
  EE_CDB EE_CONST * p_CDB, EE_SN EE_CONST ** pp_block_queue_head )
{
  ...
  /* Get the first in RQ */
  p_ready_queue = p_CCB->p_rq_first;
  ...
  EE_CDB EE_CONST * p_CDB, EE_SN EE_CONST * p_block_queue_head )
{
  ...
  p_TDB_unblocked->p_TCB->status  = EE_TASK_READY;

  rq_head_changed = EE_schedule_data_priority_insert(&p_CDB->p_CCB->p_rq_first,
  ...
  EE_CDB  EE_CONST * p_CDB, EE_TDB EE_CONST * p_TDB )
{
  ...
  p_CCB->p_curr_sn              = EE_scheduler_sn_alloc(p_CCB);
  ...
  EE_CDB  EE_CONST * p_CDB )
{
  ...
  if ( p_TDB_term != p_CDB->p_idle_task ) {
    ...
    EE_scheduler_sn_release(p_CCB, p_CCB->p_curr_sn);
    EE_unlock_core(p_CDB);
  } else {
    p_TDB_term = NULL;
  }
